...
| Integrations | Swiggy Builders Club MCP (Food, Instamart), LiveKit (WebRTC + Agents), Sarvam AI (STT/TTS), OpenAI (LLM), RevenueCat |
...
| LiveKit | Real-time WebRTC transport and Agents framework for the Talk voice loop | Cloud or self-host ops; React Native needs a custom native build. See `docs/voice-app-architecture.md`. |
| Sarvam AI | Saaras v3 streaming STT (with VAD/turn detection) and Bulbul v3 TTS via LiveKit plugins | Production-grade at scale; residual risk is end-to-end latency and LiveKit plugin integration quality, not core accuracy. |
| OpenAI | Streaming LLM inside the LiveKit Agent session (order understanding, clarification, tool calls) | Latency and cost; tool-calling must never place orders — only build carts for user confirm. |
